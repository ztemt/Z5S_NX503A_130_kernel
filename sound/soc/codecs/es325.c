...
#define ES325_INTERNAL_ROUTE_MAX 14//12
...
int es325_remote_cfg_slim_rx(int dai_id)
{
	...
	if(!es325_priv.is_fw_ready) {
		pr_info("%s(): eS325 firmware is not ready\n", __func__);
		return rc;
	}
				 ...
				 int len)
{
	...
	pr_debug("GAC:%s(len = %d); entry\n", __func__, len);
	
	if (!es325_priv.is_fw_ready) {
		pr_info("%s(): ES325 FW is not ready\n", __func__);
		return rc;
	}
...
static int es325_write_data_block_cmd(struct es325_priv *es325, u8 *cmd_blk)
{
	...
	es325_slim_read(es325, ES325_READ_VE_OFFSET,
			ES325_READ_VE_WIDTH, ack_msg, 4, 1);
	pr_debug("%s(): ack = 0x%02x%02x%02x%02x\n",
		__func__, ack_msg[0], ack_msg[1], ack_msg[2],
		ack_msg[3]);
	switch (ack_msg[3]) {
	case 0x00:
...
static void es325_ping(struct es325_priv *es325)
{
	...
	es325_slim_read(es325, ES325_READ_VE_OFFSET,
			ES325_READ_VE_WIDTH, ack_msg, 4, 1);
	pr_debug("GAC:%s(): ping ack = %02x%02x%02x%02x\n", __func__,
...
static void es325_switch_route(long route_index)
{
	...
	pr_debug("GAC:%s():switch current es325_internal_route = %ld to new route = %ld\n",
...
static ssize_t es325_live_show(struct device *dev,
               ...
               char *buf)
{
    pr_debug("%s called es325_present =%d \n", __func__,es325_present);
...
static int es325_bootup(struct es325_priv *es325)
{
	...
	pr_debug("%s(): msg[0] = 0x%02x\n", __func__, msg[0]);
	pr_debug("%s(): msg[1] = 0x%02x\n", __func__, msg[1]);
	pr_debug("%s(): msg[2] = 0x%02x\n", __func__, msg[2]);
	pr_debug("%s(): msg[3] = 0x%02x\n", __func__, msg[3]);
	rc = ES325_BUS_WRITE(es325, ES325_WRITE_VE_OFFSET,
	...
	pr_debug("%s(): buf_frames = %d", __func__, buf_frames);
...
static int fw_download(void *arg)
{
	...
	pr_debug("%s(): would release firmware here...\n", __func__);
	release_firmware(priv->fw);

	pr_debug("%s(): registering sound soc\n", __func__);
	...
	pr_debug("%s(): exiting fw_download\n", __func__);
	return 0;
}
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("%s(): es325_rx2_route_enable = %d\n", __func__,
...
{
	pr_debug("GAC:%s(dai->name = %s): entry\n", __func__, dai->name);
	pr_debug("GAC:%s(dai->id = %d): entry\n", __func__, dai->id);
...
static int es325_put_internal_route_config(struct snd_kcontrol *kcontrol,
					   struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("GAC:%s():new route index = %ld\n", __func__, ucontrol->value.integer.value[0]);
...
static const char * const es325_get_route_change_status_enum_texts[] = {
	"Active", "Muting", "Switching", "Unmuting", "Inactive"
};
static const struct soc_enum es325_get_route_change_status_enum=
	SOC_ENUM_SINGLE(ES325_CHANGE_STATUS, 0,
			ARRAY_SIZE(es325_get_route_change_status_enum_texts),
			es325_get_route_change_status_enum_texts);
...
static struct snd_kcontrol_new es325_digital_ext_snd_controls[] = {
	...
	SOC_ENUM_EXT("ES325 Get Route Change Status", es325_get_route_change_status_enum,
		     es325_get_control_enum, es325_put_control_enum),
			       ...
			       unsigned int *tx_slot, unsigned int rx_num, unsigned int *rx_slot)
{
	...
	if (id == ES325_SLIM_1_PB ||
		...
		es325->dai[DAI_INDEX(id)].ch_act = 0;
		pr_debug("GAC:%s() id = %d\n", __func__, id);
		pr_debug("GAC:%s() ch_tot = %d\n", __func__, tx_num);
		for (i = 0; i < tx_num; i++) {
			es325->dai[DAI_INDEX(id)].ch_num[i] = tx_slot[i];
			pr_debug("GAC:%s() tx_slot[] = %d\n", __func__, tx_slot[i]);
		}
	}
...
static int es325_slim_probe(struct slim_device *sbdev)
{
	...
	gpio_set_value(pdata->wakeup_gpio, 1);
#else
	rc = gpio_direction_input(pdata->wakeup_gpio);
	if (rc < 0) {
		dev_err(&sbdev->dev, "%s(): es325_wakeup direction failed",
			__func__);
		goto wakeup_gpio_direction_error;
...
static ssize_t es325_status_show(struct device *dev, 
...
static DEVICE_ATTR(es325_status, 0444, es325_status_show, NULL);
static struct kobject *es325_kobject = NULL;

static int es325_attribute_init(void)
{
	int ret;

	es325_kobject = kobject_create_and_add("ztemt_es325_status", NULL);
	if(es325_kobject == NULL) {
		ret = -ENOMEM;
		goto err1;
	}

	ret = sysfs_create_file(es325_kobject, &dev_attr_es325_status.attr);
	if(ret){
		goto err;
	}

	gpio_request(92, "es325_power");
	gpio_tlmm_config(GPIO_CFG(92, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
	
	es325_exist = __gpio_get_value(92);
	if (es325_exist == 1)
	{
		printk("%s(): -----es325 chip exist\n", __func__);
	}
	else
	{
		printk("%s(): -----es325 chip not exist\n", __func__);
	}
	gpio_tlmm_config(GPIO_CFG(92, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
	gpio_free(92);
	return 0;

err:
	kobject_del(es325_kobject);
err1:
	printk("es325_status: Failed to create sys file\n");
	return ret;
}
